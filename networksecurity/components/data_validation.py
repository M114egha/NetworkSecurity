...
class DataValidation:
    ...
    def validate_no_of_columns(self , dataframe:pd.DataFrame )->bool:
        try:
            ...
            logging.info(f"Required no of cols:{number_of_columns}")
            logging.info(f"Dataframe has columns:{len(dataframe.columns)}")
        except Exception as e:
            raise NetworkSecurityException(e, sys)
        
    
    def detect_dataset_drift_report(self , base_df , current_df , threshold=0.05)->bool:
        try:
            status=True
            report={}
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            #Read the data from train and test fiels
            ...
            #Chceking datadrift
            status=self.detect_dataset_drift_report(base_df=train_dataframe , current_df= test_dataframe)
